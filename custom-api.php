...
function callMatomo($params, $userExistCall = false)
	{
	$response = $GLOBALS['client']->get(dirname($_SERVER['PHP_SELF']) , ['query' => $params]);
...
// ------------ Adding New Site end----------------//
// ---- If User Already have Super user access ignore set access ----- //

$params = $defaultParams;
$params["method"] = "UsersManager.hasSuperUserAccess";
$params["token_auth"] = $userToken;
$isSuperUser = callMatomo($params);
...
// ---- If User Already have Super user access ignore set access end ----- //
